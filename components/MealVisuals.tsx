  ...
  useEffect(() => {
    ...
    meal?.items
      ? meal.items.forEach((item: MealItem) => {
          ...
          let targetItem: ConsumptionByItem | undefined =
            targetClass?.items?.find((el2: ConsumptionByItem) => {
  ...
  return (
    <>
      <div className={styles.emissionVisual}>
        ...
        <div>
          {emissions.toFixed(2)} kg CO<sub>2</sub>
        </div>
      </div>
      <div className={styles.emissionVisual}>
        <div style={{ color: "#587c0c" }}>Land Use</div>
        <div>
          {landUse.toFixed(2)} m<sup>2</sup> year
        </div>
      </div>
      <div className={styles.emissionVisual}>
        <div style={{ color: "#296c7d" }}>Water Use</div>
        <div>
          {waterUse.toFixed(2)} L {nan ? "*" : null}
        </div>
      </div>
    </>
  );
